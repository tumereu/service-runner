use crate::config::{
    AutomationDefinitionId, ProfileDefinition, ServiceDefinition, ServiceId, TaskDefinition,
    TaskDefinitionId, TaskStep,
};
use crate::models::task::Task;
use crate::models::{Automation, Service, TaskId};
use log::error;
...
impl Profile {
    pub fn new(profile: ProfileDefinition, all_services: &Vec<ServiceDefinition>) -> Profile {
        let services: Vec<Service> = profile
            .services
            .iter()
        ...
        let all_task_definitions: Vec<(TaskDefinition, Option<ServiceId>)> = profile
            .tasks
            .iter()
            .map(|task_def| (task_def.clone(), None))
            .chain(services.iter().flat_map(|service| {
                service
                    .definition
                    .tasks
                    .iter()
                    .map(|task_def| (task_def.clone(), Some(service.definition.id.clone())))
            }))
            .collect();

        let profile_automations: Vec<Automation> = profile
            .automation
            .iter()
            .map(|automation| (automation.clone(), None).into())
            .collect();
