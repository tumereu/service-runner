...
impl Component for OutputDisplay {
    ...
    fn render(self, context: &mut FrameContext) -> UIResult<Self::Output> {
        ...
        // element?
        let mut lines: Vec<Vec<LinePart>> = self
            .lines
            .iter()
            .flat_map(|line| {
                if self.wrap {
                    ...
                    if line
                        .parts
                        .iter()
                        .all(|part| part.text.chars().all(|char| char.is_whitespace()))
                    {
                        lines.push(line.prefix.to_vec());
                    } else {
                        line.parts
                            .iter()
                            .flat_map(|part| {
                                ...
                                UnicodeSegmentation::graphemes(text.as_str(), true).for_each(
                                    |grapheme| {
                            ...
                            .for_each(|part| {
                                let required_width: usize = lines
                                    .last()
                                    .iter()
                                ...
                                if lines.is_empty() || required_width > size.width.into() {
                                    ...
                                    if part.text.chars().any(|char| !char.is_whitespace())
                                        || lines.is_empty()
                                    {
                                        lines.push(
                                            line.prefix
                    ...
                    vec![
                        line.prefix
                            ...
                            .map(|part| {
                                ...
                                remaining_to_drop =
                                    remaining_to_drop.saturating_sub(part.text.len());
                                new_part
                            })
                            .collect(),
                    ]
