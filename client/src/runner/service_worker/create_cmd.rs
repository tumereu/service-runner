...
use subst::Error;
use subst::error::{
    InvalidEscapeSequence, MissingClosingBrace, MissingVariableName, UnexpectedCharacter,
};

#[derive(Debug, Error)]
pub enum CmdCreationError {
    NoSuchVariable { var: String, target: String },
    MalformattedExpression { explanation: String, target: String },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CmdCreationError::NoSuchVariable { var, target } => {
                write!(
                    f,
                    "Error in expression '{target}': variable '{var}' is not defined"
                )
            }
        }
    }
...
fn env_subst(value: &str) -> Result<String, CmdCreationError> {
    subst::substitute(value, &subst::Env).map_err(|err| match err {
        Error::InvalidEscapeSequence(InvalidEscapeSequence { position, .. }) => {
            MalformattedExpression {
                explanation: format!(
                    "Expression contains an invalid escape sequence at position {position}"
                ),
                target: value.to_string(),
            }
        }
        Error::MissingVariableName(MissingVariableName { position, .. }) => {
            MalformattedExpression {
                explanation: format!(
                    "Expression is missing a variable name at position {position}"
                ),
                target: value.to_string(),
            }
        }
        Error::UnexpectedCharacter(UnexpectedCharacter { position, .. }) => {
            MalformattedExpression {
                explanation: format!(
                    "Expression contains an unexpected character at position {position}"
                ),
                target: value.to_string(),
            }
        }
